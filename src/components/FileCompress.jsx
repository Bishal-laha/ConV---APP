...
import { setIsCompressFileDrawerOpen } from '../store/slice/componentSlice';

function FileCompress() {
    const dispatch = useDispatch();
    const { isCompressFileDrawerOpen } = useSelector((state) => state.component);
    const { fileUrl, fileMetaData } = useSelector((state) => state.file);
    ...
    const handleCompress = async () => {
        try {
            if (fileUrl) {
                ...
                dispatch(setIsCompressFileDrawerOpen(false));
            }
        } catch (error) {
    ...
    return (
        <Dialog open={isCompressFileDrawerOpen} fullWidth>
            <DialogTitle sx={{ textAlign: "center" }}>Compress PDF</DialogTitle>
            <IconButton sx={{ position: "absolute", top: "12px", right: "10px" }} onClick={() => dispatch(setIsCompressFileDrawerOpen(false))}>
